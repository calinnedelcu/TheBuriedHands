...
@export_group("Epilogue Auto-Walk")
## Daca true: dupa collapse, player-ul se ridica, merge automat inainte si
## la final declanseaza ending-ul via ExitLightGate.trigger_sequence().
@export var enable_epilogue_walk: bool = true
@export var epilogue_pause_after_collapse: float = 2.4
## Cat timp merge inainte automat inainte sa declanseze ending-ul.
@export var epilogue_walk_duration: float = 7.0
...
func _collapse_after_exit(player: Node) -> void:
	...
	if enable_epilogue_walk:
		_run_epilogue(player)

func _run_epilogue(player: Node) -> void:
	if epilogue_pause_after_collapse > 0.0:
		await get_tree().create_timer(epilogue_pause_after_collapse).timeout
	if player == null or not is_instance_valid(player):
		return
	if player.has_method("force_stand_up"):
		player.call("force_stand_up")
	if player.has_method("set_auto_walk_forward"):
		player.call("set_auto_walk_forward", true)
	await get_tree().create_timer(epilogue_walk_duration).timeout
	if is_instance_valid(player) and player.has_method("set_auto_walk_forward"):
		player.call("set_auto_walk_forward", false)
	# Declanseaza ending-ul via API-ul ExitLightGate (gasit prin grup).
	var gate := get_tree().get_first_node_in_group("exit_light_gate")
	if gate != null and gate.has_method("trigger_sequence"):
		gate.call("trigger_sequence", player)
...
func _trigger_camera_shake(player: Node) -> void:
	if player != null and player.has_method("play_camera_shake"):
		player.call("play_camera_shake", 1.2, 0.14)
